...
public class TBScheduleManagerFactory implements ApplicationContextAware {
	...
	public void setZkConfig(Map<String,String> zkConfig) {
		...
		final String zkConnectString ="zkConnectString";
		final String zkHead ="zookeeper://";
		if (zkConfig!=null &&! zkConfig.isEmpty())	{
		    String zkAdress =zkConfig.get(zkConnectString);
    		if (zkAdress!=null && zkAdress.startsWith(zkHead)){
    		    zkAdress =zkAdress.substring(zkHead.length());
    		    zkConfig.put(zkConnectString, zkAdress);
    		}
		}
	}
